...
\end{figure}
As we see, the contrast is worsened by adding the high pass filtered version of the image on top of the original. To combat this, we can apply histogram equalization afterwards, producing the results shown in figure \ref{fig:moon_sharpened_good}. This more clearly showns that the image has been sharpened. The original image has quite low contrast to begin with, so for fair comparison, we apply histogram equalization to that too.
\begin{figure}[H]
\centering
